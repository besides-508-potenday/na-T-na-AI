...
REACT_IMPROVED_PARAMS = ALL_PARAMS["REACT_IMPROVED"]
...
def generate_situation_and_quiz():
    ...
    - Do not generate situation that don't fit the context. ex. "생일 파티에 초대받았는데, 친구가 내 생일을 잊어버린 것 같아."
...
def generate_response(conversation, score, chatbot_name, user_nickname, quiz_list):
    ref = ""
    ...
    TASK 2: Improve a sentence so that it flows naturally.
    - The sentence is what you should say after repond (task1). 
    - Consider the previous conversation.
    - This is the sentence: "{default_question}"

    Return "respond" and "improved_sentence" as JSON FORMAT.
    """
    ...
    try:
        ...
        while True:
            ...
            if attempt > 0:
                result = execute_react(system_message_react_and_improved + f"Generate the react and improved_sentence with {MAX_REACT_LENGTH*0.7} characters or less.\n", conversation[-1], REACT_PARAMS)
            else:
                result = execute_react(system_message_react_and_improved, conversation[-1],  REACT_IMPROVED_PARAMS)
            if result:
                # print(f"{result['response_text']}")
                json_str = result['response_text']
                json_str = json.loads(json_str)
                respond = json_str['respond']
                improved_sentence = json_str['improved_sentence']

                if respond[:len(chatbot_name)] == chatbot_name:
                    respond = respond[len(chatbot_name)+1:].strip()

            if len(respond) <= MAX_REACT_LENGTH and len(improved_sentence) <= MAX_REACT_LENGTH:
                print(f"리액션: {respond}")
                print(f"리액션 길이: {len(respond)}")
                print(f"기존 퀴즈:{default_question}")
                print(f"개선된 퀴즈: {improved_sentence}")
                print(f"개선된 퀴즈 길이: {len(improved_sentence)}")
                return respond, improved_sentence
